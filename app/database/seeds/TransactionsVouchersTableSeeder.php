...
class TransactionsVouchersTableSeeder extends DatabaseSeeder
{
  public function run()
  {
    ...
    $interpreter->addObserver(function(array $columns) use ($pdo) {
      try{
        $columns[1] = ($columns[1] == 'NULL') ? NULL : $columns[1];
        $stmt = $pdo->prepare('INSERT INTO transactions_vouchers (voucher_id, payment_partner_id, transaction_id, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)');
    ...
    $lexer->parse(app_path().'/database/seeds/assets/ImportacaoBDNovo/transactions_vouchers.csv', $interpreter);
  }
}
