...
class OffersTableSeeder extends DatabaseSeeder
{
  public function run()
  {
    ...
    // $config->setToCharset('ISO-8859-1');
    ...
    $interpreter->addObserver(function(array $columns) use ($pdo) {
      try{
        $columns[4] = ($columns[4] == 'NULL') ? NULL : $columns[4];
        $columns[5] = ($columns[5] == 'NULL') ? NULL : $columns[5];
        $stmt = $pdo->prepare('INSERT INTO offers (id, partner_id, ngo_id, category_id, title, subtitle, features, destiny_id, rules, starts_on, ends_on, slug, display_order, created_at, deleted_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
