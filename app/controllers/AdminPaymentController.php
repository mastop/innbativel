...
class AdminPaymentController extends BaseController {
	...
	public function anyVoucher()
	{
		...
		$transactionVoucherData = $transaction_voucher->with(['voucher' => function($query){ 
													  ...
													  // ->limit(50)->get()->toArray(); print('<pre>'); print_r($transactionVoucherData); print('</pre>'); die();
		...
		$ppvoucherData = $ppvoucher->with(['voucher_offer_order', 'payment_partner'])
					   ...
					   //              if (Input::has('name')) {
						  ...
						  //                     ->from('profiles')
								...
								// 			  ->whereRaw('CONCAT(CONCAT(COALESCE(profiles.first_name, ""), " ", COALESCE(profiles.last_name, "")), " ", profiles.company_name) LIKE "%'.str_replace("'", "\'", Input::get('name')).'%"');
								// 	}
 ...
 //    	}
	...
	// 					                      ->from('profiles')
	// 										  ->whereRaw('profiles.user_id = orders.user_id')
	// 										  ->whereRaw('CONCAT(CONCAT(COALESCE(profiles.first_name, ""), " ", COALESCE(profiles.last_name, "")), " ", profiles.company_name) LIKE "%'.str_replace("'", "\'", $name).'%"');
	// 								}
